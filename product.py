...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.lineEdit.setGeometry(QtCore.QRect(70, 770, 500, 41))
        self.lineEdit1 = QtWidgets.QLineEdit(Form)
        self.lineEdit1.setGeometry(QtCore.QRect(580, 770, 490, 41))
        self.lineEdit2 = QtWidgets.QLineEdit(Form)
        self.lineEdit2.setGeometry(QtCore.QRect(1080, 770, 500, 41))
        ...
        self.label_4.setGeometry(QtCore.QRect(500, 120, 241, 81))
        font = QtGui.QFont()
    ...
    def retranslateUi(self, Form):
        ...
        item.setText(_translate("Form", "产品尺寸"))
        item = self.tableWidget.horizontalHeaderItem(2)
        item.setText(_translate("Form", "刀模编号"))
        item = self.tableWidget.horizontalHeaderItem(3)
        item.setText(_translate("Form", "查看产品"))
        self.pushButton_2.setText(_translate("Form", "删除产品"))
        self.pushButton_3.setText(_translate("Form", "增加产品"))
        self.pushButton_4.setText(_translate("Form", "更新产品"))
    ...
    def refreshByDB(self):
        ...
        if(cursor):
            ...
            for row in cursor:
                for j in range(4):
                    if j < 3:
                        self.tableWidget.setItem(i, j, QtWidgets.QTableWidgetItem(row[j+1]))
                    else:
                        # print(row[0])
                        self.tableWidget.setCellWidget(i, 3, self.buttonForRow(i))
                i += 1
        conn.close()

    def product_operation(self,op):
        ...
        if(op=="delete"):
            ...
            if (len(row) == 0):
                c.execute("INSERT INTO PRODUCT (GNAME,PNAME,SIZE,NUMBER) \
                      VALUES ({mygname},{mypname},{mysize}, {mynumber})" \
                          .format(mygname=guestName, mypname=product, mysize=size, mynumber=number))
                reply = QMessageBox.information(self,  # 使用infomation信息框
                                                "通知",
                                                "增加成功！")
            else:
            ...
            if (len(row) > 0):
                ...
                if (len(number) == 2): number = "'" + row[0][2] + "'"
                c.execute("UPDATE PRODUCT set SIZE={mysize}, NUMBER={mynumber} \
                           where GNAME={mygname} AND PNAME={mypname}".format(mygname=guestName,mypname=product,mysize=size,mynumber=number))
                reply = QMessageBox.information(self,  # 使用infomation信息框
                                                "通知",
                                                "更新成功！")
            else:
        ...
        cursor = c.execute("SELECT COUNT(NAME) FROM   PRODUCT")
        for row in cursor:
            self.tableWidget.setRowCount(row[0])
        conn.close()
        self.refreshByDB()
...
class mywindow(QtWidgets.QWidget,Ui_Form):      #实例化窗口
    def __init__(self):
        ...
        #对事件和响应函数建立连接
        self.pushButton_2.clicked.connect(lambda: self.product_operation("delete"))
        self.pushButton_3.clicked.connect(lambda: self.product_operation("add"))
        self.pushButton_4.clicked.connect(lambda: self.product_operation("refresh"))
